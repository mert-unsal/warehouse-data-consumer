...
public class InventoryEventErrorConsumer {
    ...
    @KafkaListener(
            topics = "${KAFKA_TOPIC_INVENTORY_ERROR:ikea.warehouse.inventory.update.topic.error}",
    ...
    public void consume(InventoryUpdateEvent inventoryUpdateEvent, Acknowledgment ack) {
        log.error("Error Consumer - event detail: {}",inventoryUpdateEvent.toString());
        ack.acknowledge();
    }
